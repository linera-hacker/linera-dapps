...
pub fn uq128_encode_divisor_div_then_mul_to_big_amount(
    divisor: Amount,
    dividend: Amount,
    extra: Amount,
) -> BigAmount {
    let _divisor = BigUint::from_u128(u128::from(divisor)).expect("Couldn't convert amount");
    let uq64_divisor = BigUint::from_u128(2u128.pow(64)).expect("Couldn't convert amount");
